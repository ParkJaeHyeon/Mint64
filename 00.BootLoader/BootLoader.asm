...
jmp 0x07C0:START				; CS 세그먼트 레지스터에 0x07C0을 복사하면서 
						;START 레이블로 이동
...
START:
	...
	mov ax, 0xB800				; 비디오 메모리의 시작 어드레스(0xB800)를 
						;세그먼트 레지스터 값으로 변환
...
.SCREENCLEARLOOP:				; 화면을 지우는 루프
	...
	jl .SCREENCLEARLOOP			; SI 레지스터가 80 * 25 * 2 보다 작다면 아직 지우지 못한
...
READDATA:					; 디스크를 읽는 코드의 시작
	...
	mov dl, 0x00				; 읽을 드라이브 번호( 0 = Flopy) 설정
	int 0x13				; 인터럽트 서비스 수행
	jc HANDLEDISKERROR			; 에러가 발생헀다면 HANDLEDISKERROR로 이동

	; 복사할 어드레스와 트랙, 헤드, 섹터 어드레스 계산
	
	add si, 0x0020				; 512(0x200)바이트만큼 읽었으므로, 이를 세그먼트
						; 레지스터값으로 변환
...
READEND:
	...
	push LOADINGCMPLETEMESSAGE		; 출력할 메시지의 어드레스를 스택에 삽입
	push 1					; 화면 Y 좌표(1)를 스택에 삽입
...
PRINTMESSAGE:
	...
	mov bp, sp				; 베이스 포인터 레지스터(BP)에 스택 포인터 레지스터(SP)의
						; 값을 설정, 베이스 포인터 레지스터(BP)를 이용해서 파라미터
						; 에 접근할 목적

	push es					; ES 세그먼트 레지스터부터 DX 레지스터까지 스택에 삽입
	push si					; 함수에서 임시로 사용하는 레지스터로 함수의 마지막 부분
						; 에서 스택에 삽입된 값을 꺼내 원래 값으로 복원
	push di
	push ax
	push cx
	push dx
	...
	mov ax, 0xB800				; 비디오 메모리 시작 어드레스(0x0B8000)를 세그먼트
						; 레지스터 값으로 변환
	mov es, ax				; ES 세그먼트 레지스터에 설정
	...
	add di, ax				; 화면 Y 어드레스와 계산된 X 어드레스를 더해서 실제
						; 비디오 메모리 어드레스를 계산
	; 출력할 문자열의 어드레스
...
.MESSAGELOOP:					; 메시지를 출력하는 루프
	mov cl, byte [ si ]			; SI 레지스터가 가리키는 문자열 위치에서 한 문자를
						...
						; 문자열은 1바이트면 출분하므로 CX 레지스터의 하위
						; 1 바이트만 사용

	cmp cl, 0				; 복사된 문자와 0을 비교
	je .MESSAGEEND				; 복사한 문자의 값이 0이면 문자열이 종료되었음을 의미
						; 하므로 .MESSAGEEND로 이동하여 문자 출력 종료
	
	mov byte [ es:di ], cl			; 0이 아니라면 비디오 메모리 어드레스 0xB800:di에 문자를 출력
	
	add si, 1				; SI 레지스터에 1을 더하여 다음 문자열로 이동
	add di, 2				; DI 레지스터에 2를 더하여 비디오 메모리의 다음 문자 위치로
						; 이동, 비디오 메모리는 (문자, 속성)의 쌍으로 구성되므로
						; 문자만 출력하려면 2를 더해야 함

	jmp .MESSAGELOOP			; 메시지 출력 루프로 이동하여 다음 문자를 출력
	
.MESSAGEEND:
	pop dx					; 함수에서 사용이 끝난 DX 레지스터부터 ES 레지스터까지를 스택
	pop cx					; 에 삽입된 값을 이용해서 복원
	pop ax					; 스택은 가장 마지막에 들어간 데이터가 가장 먼저 나오는
	pop di					; 자료구조(LIFO)이므로 삽입(push)의 역순으로 제거(pop)
	pop si					; 해야 함
	pop es
...
MESSAGE1:	db 'MINT64 OS Boot Loader Start~!!', 0	; 출력할 메시지 정의
							; 마지막은 0으로 설정하여 .MESSAGELOOP에서 문자열
							; 이 종료되었음을 알 수 있도록 함
DISKERRORMESSAGE:	db 'DISK Error~!!', 0
IMAGELOADINGMESSAGE:	db 'OS Image Loading...', 0
LOADINGCMPLETEMESSAGE: 	db 'Complete~!!', 0

; 디스크 읽기에 관련된 변수들
SECTORNUMBER:		db 0x02			; OS 이미지가 시작하는 섹터 번호를 지정하는 영역
...
db 0xAA						; 1바이트를 선언하고 값은 0xAA
						; 어드레스 511, 512에 0x55, 0xAA를 써서 부트 섹트로 표기
