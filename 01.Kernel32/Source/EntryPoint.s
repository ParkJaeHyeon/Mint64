...
PRINTMESSAGE:
	...
	push esi	; 함수에서 임시로 사용하는 레지스터로 함수의 마지막 부분에서 스택에
	push edi	; 삽입된 값을 꺼내 원래 값으로 복원
	push eax
	push ecx
	push edx
...
.MESSAGEEND:
	...
	pop eax		; 스택은 가장 마지막에 들어간 데이터가 가장 먼저 나오는 자료구조이므로
	pop edi		; 삽입의 역수능로 제거해야 함
	pop esi	
